...
const API = '/reference/application-types';

export default function AdminAppTypesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['ref-application-types'] }); closeDialog(); },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['ref-application-types'] }); setDeleteId(null); },
  });
  ...
  const openEdit = useCallback((item: AppTypeItem) => {
    setEditItem(item);
    setName(item.name);
    setDialogOpen(true);
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage product application types</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card><p className="py-8 text-center text-gray-400">No application types found</p></Card>
      ) : (
        <DataTable columns={columns} data={items as unknown as Record<string, unknown>[]} />
      )}

      <Dialog open={dialogOpen} onClose={closeDialog} title={editItem ? 'Edit Application Type' : 'Add Application Type'} className="w-full max-w-sm">
