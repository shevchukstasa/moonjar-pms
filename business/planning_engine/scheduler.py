...
def generate_production_schedule(
    ...
    for pos in positions:
        ...
        _pos_done_idx = _position_completed_index(pos)

        if _plan and isinstance(_plan, dict):
            # New path: distribute across full stage durations — but skip
            # stages the position has already passed per its current status.
            for _sname in _GLAZING_STAGES:
                _sinfo = _plan.get(_sname)
                if not _sinfo:
                    continue
                if _should_hide_stage(pos, _sname):
                    continue
                _expand_stage(_sname, _sinfo, glazing_by_date, pos)
            # kiln_loading is a single-day event on planned_kiln_date
            if pos.planned_kiln_date and not _should_hide_stage(pos, "kiln_loading"):
            ...
            # Legacy path — single-day buckets (no stage_plan yet). Map
            # each planned_*_date to the stage it represents and hide it
            # if the position is already past that stage.
            if pos.planned_glazing_date and _pos_done_idx < _STAGE_INDEX["glazing"]:
                # planned_glazing_date covers the unpacking→glazing block;
                # once glazing is done (idx=4), don't paint this bucket.
                glazing_by_date.setdefault(pos.planned_glazing_date, []).append((pos, None))
            if pos.planned_kiln_date and _pos_done_idx < _STAGE_INDEX["kiln_loading"]:
                kiln_by_date.setdefault(pos.planned_kiln_date, []).append((pos, None))
            if pos.planned_sorting_date and _pos_done_idx < _STAGE_INDEX["sorting"]:
                sorting_by_date.setdefault(pos.planned_sorting_date, []).append((pos, None))
            if pos.planned_completion_date and _pos_done_idx < _STAGE_INDEX["packing"]:
