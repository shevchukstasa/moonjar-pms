...
const RANK_DECORATIONS = [
  { emoji: '\u{1F947}', ring: 'ring-amber-400 dark:ring-amber-500', glow: 'shadow-amber-200/60 dark:shadow-amber-500/20' },
  { emoji: '\u{1F948}', ring: 'ring-gray-400 dark:ring-gray-500', glow: 'shadow-gray-200/60 dark:shadow-gray-500/20' },
  { emoji: '\u{1F949}', ring: 'ring-orange-400 dark:ring-orange-600', glow: 'shadow-orange-200/60 dark:shadow-orange-600/20' },
];
      ...
      {selectedCompetition && (
        <Card variant="glass" className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex gap-3">
            <StatPill icon="\u{23F3}" label="Days left" value={daysLeft(selectedCompetition.end_date)} />
            {selectedCompetition.prize_budget_idr && (
              <StatPill icon="\u{1F3C6}" label="Prize pool" value={formatIDR(selectedCompetition.prize_budget_idr)} />
            )}
          </div>
        </Card>
      )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-3 mt-0.5">
            ...
            {standing.streak_days > 0 && (
              <span className="text-xs text-orange-500 dark:text-orange-400">{'\u{1F525}'} {standing.streak_days}d streak</span>
            )}
          </div>
        </div>
      ...
      {Object.entries(grouped).map(([category, badgeList]) => (
        <div key={category}>
          <h3 className="mb-3 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-gray-500 dark:text-stone-500">
            <span>{SKILL_CATEGORY_ICONS[category] || '\u{1F4DA}'}</span>
            {category}
          </h3>
      ...
      <div className={cn(
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <span className="text-2xl">{badge.icon || SKILL_CATEGORY_ICONS[badge.category] || '\u{1F4DA}'}</span>
            <div>
...
function PrizesTab({ factoryId }: { factoryId: string }) {
  ...
  return (
    <FadeIn className="space-y-6">
      ...
      {pending.length > 0 && (
        <PrizeSection
          title={`\u{23F3} Pending Approval (${pending.length})`}
          prizes={pending}
          isOwner={!!isOwner}
      ...
      {approved.length > 0 && (
        <PrizeSection
          title={`\u{2705} Approved (${approved.length})`}
          prizes={approved}
          isOwner={!!isOwner}
      ...
      {/* Already awarded */}
      {awarded.length > 0 && (
        <PrizeSection title={`\u{1F3C6} Awarded (${awarded.length})`} prizes={awarded} />
      )}

      {/* Rejected */}
      {rejected.length > 0 && (
        <PrizeSection title={`\u{274C} Rejected (${rejected.length})`} prizes={rejected} />
      )}
    </FadeIn>
  );
}
  ...
  return (
    <FadeIn className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiCard icon="\u{2B50}" label="Points This Month" value={dash.total_points_this_month.toLocaleString()} />
        <KpiCard icon="\u{1F465}" label="Active Users" value={dash.active_users} />
        <KpiCard icon="\u{1F3C6}" label="Active Competitions" value={dash.active_competitions} />
        <KpiCard icon="\u{1F4AA}" label="Skills Certified" value={dash.skills_certified_this_month} />
      </div>
      ...
      {dash.top_performers?.length > 0 && (
        <Card>
          <h3 className="mb-3 text-sm font-bold text-gray-900 dark:text-stone-100">{'\u{1F31F}'} Top Performers</h3>
          <div className="space-y-2">
      ...
      {!loadingImpact && impact && (
        <Card>
          <h3 className="mb-3 text-sm font-bold text-gray-900 dark:text-stone-100">{'\u{1F4C8}'} Productivity Impact</h3>
      ...
      {dash.monthly_trend?.length > 0 && (
        <Card>
          <h3 className="mb-3 text-sm font-bold text-gray-900 dark:text-stone-100">{'\u{1F4CA}'} Monthly Trend</h3>
      ...
      {seasons && seasons.length > 0 && (
        <Card>
          <h3 className="mb-3 text-sm font-bold text-gray-900 dark:text-stone-100">{'\u{1F30D}'} Seasons</h3>
          <div className="space-y-2">
            {seasons.map((s) => (
