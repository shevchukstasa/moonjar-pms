...
class Factory(Base):
    ...
    kiln_constants_mode = Column(sa.Enum(KilnConstantsMode), nullable=False, default=KilnConstantsMode.MANUAL)
...
class User(Base):
    ...
    language = Column(sa.Enum(LanguagePreference), nullable=False, default=LanguagePreference.EN)
    is_active = Column(sa.Boolean, nullable=False, default=True)
...
class Supplier(Base):
    ...
    address = Column(sa.Text)
    material_types = Column(sa.ARRAY(sa.Enum(MaterialType)))
...
class SupplierLeadTime(Base):
    ...
    material_type = Column(sa.Enum(MaterialType), nullable=False)
...
class ReferenceAuditLog(Base):
    ...
    action = Column(sa.Enum(ReferenceAction), nullable=False)
...
class ProductionOrder(Base):
    ...
    status = Column(sa.Enum(OrderStatus), nullable=False, default=OrderStatus.NEW)
    status_override = Column(sa.Boolean, nullable=False, default=False)
    sales_status = Column(sa.String(100))
    source = Column(sa.Enum(OrderSource), nullable=False, default=OrderSource.MANUAL)
...
class ProductionOrderItem(Base):
    ...
    product_type = Column(sa.Enum(ProductType), nullable=False, default=ProductType.TILE)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class ProductionOrderChangeRequest(Base):
    ...
    status = Column(sa.Enum(ChangeRequestStatus), nullable=False, default=ChangeRequestStatus.PENDING)
...
class ProductionOrderStatusLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    order_id = Column(UUID(as_uuid=True), ForeignKey('production_orders.id', ondelete='CASCADE'), nullable=False)
    old_status = Column(sa.Enum(OrderStatus))
    new_status = Column(sa.Enum(OrderStatus), nullable=False)
    changed_by = Column(UUID(as_uuid=True), ForeignKey('users.id'))
...
class Resource(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    name = Column(sa.String(200), nullable=False)
    resource_type = Column(sa.Enum(ResourceType), nullable=False)
    factory_id = Column(UUID(as_uuid=True), ForeignKey('factories.id'), nullable=False)
    capacity_sqm = Column(sa.Numeric(10, 3))
    capacity_pcs = Column(sa.Integer)
    num_levels = Column(sa.Integer, default=1)
    status = Column(sa.Enum(ResourceStatus), nullable=False, default=ResourceStatus.ACTIVE)
...
class Batch(Base):
    ...
    status = Column(sa.Enum(BatchStatus), nullable=False, default=BatchStatus.PLANNED)
    created_by = Column(sa.Enum(BatchCreator), nullable=False, default=BatchCreator.AUTO)
    notes = Column(sa.Text)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    updated_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class ScheduleSlot(Base):
    ...
    status = Column(sa.Enum(ScheduleSlotStatus), nullable=False, default=ScheduleSlotStatus.PLANNED)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class OrderPosition(Base):
    ...
    factory_id = Column(UUID(as_uuid=True), ForeignKey('factories.id'), nullable=False)
    status = Column(sa.Enum(PositionStatus), nullable=False, default=PositionStatus.PLANNED)
    batch_id = Column(UUID(as_uuid=True), ForeignKey('batches.id'))
    ...
    place_of_application = Column(sa.String(50))
    product_type = Column(sa.Enum(ProductType), nullable=False, default=ProductType.TILE)
    shape = Column(sa.Enum(ShapeType), default=ShapeType.RECTANGLE)
    thickness_mm = Column(sa.Numeric(5, 1), nullable=False, default=11.0)
    recipe_id = Column(UUID(as_uuid=True), ForeignKey('recipes.id'))
    mandatory_qc = Column(sa.Boolean, nullable=False, default=False)
    split_category = Column(sa.Enum(SplitCategory))
...
class Task(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    factory_id = Column(UUID(as_uuid=True), ForeignKey('factories.id'), nullable=False)
    type = Column(sa.Enum(TaskType), nullable=False)
    status = Column(sa.Enum(TaskStatus), nullable=False, default=TaskStatus.PENDING)
    assigned_to = Column(UUID(as_uuid=True), ForeignKey('users.id'))
    assigned_role = Column(sa.Enum(UserRole))
...
class MaterialTransaction(Base):
    ...
    related_order_id = Column(UUID(as_uuid=True), ForeignKey('production_orders.id'))
    related_position_id = Column(UUID(as_uuid=True), ForeignKey('order_positions.id'))
    reason = Column(sa.Enum(WriteOffReason))
    notes = Column(sa.Text)
...
class QualityCheck(Base):
    ...
    stage = Column(sa.Enum(QcStage), nullable=False)
    result = Column(sa.Enum(QcResult), nullable=False)
...
class GrindingStock(Base):
    ...
    status = Column(sa.Enum(GrindingStatus), nullable=False, default=GrindingStatus.IN_STOCK)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    updated_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class RepairQueue(Base):
    ...
    source_order_id = Column(UUID(as_uuid=True), ForeignKey('production_orders.id'))
    source_position_id = Column(UUID(as_uuid=True), ForeignKey('order_positions.id'))
    status = Column(sa.Enum(RepairStatus), nullable=False, default=RepairStatus.IN_REPAIR)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class ManuShipment(Base):
    ...
    status = Column(sa.Enum(ManuShipmentStatus), nullable=False, default=ManuShipmentStatus.PENDING)
...
class SurplusDisposition(Base):
    ...
    disposition_type = Column(sa.Enum(SurplusDispositionType), nullable=False)
    size = Column(sa.String(50), nullable=False)
    color = Column(sa.String(100), nullable=False)
...
class CastersBox(Base):
    ...
    removed_reason = Column(sa.Enum(CastersRemovedReason))
    notes = Column(sa.Text)
...
class TpsShiftMetric(Base):
    ...
    status = Column(sa.Enum(TpsStatus), nullable=False, default=TpsStatus.NORMAL)
    notes = Column(sa.Text)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class TpsDeviation(Base):
    ...
    shift = Column(sa.Integer, nullable=False)
    stage = Column(sa.String(100), nullable=False)
    deviation_type = Column(sa.Enum(TpsDeviationType), nullable=False)
    description = Column(sa.Text, nullable=False)
...
class BottleneckConfig(Base):
    ...
    batch_mode = Column(sa.Enum(BatchMode), nullable=False, default=BatchMode.HYBRID)
...
class BufferStatus(Base):
    ...
    buffer_health = Column(sa.Enum(BufferHealth), nullable=False, default=BufferHealth.GREEN)
    updated_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class KilnMaintenanceSchedule(Base):
    ...
    status = Column(sa.Enum(MaintenanceStatus), nullable=False, default=MaintenanceStatus.PLANNED)
    notes = Column(sa.Text)
    created_by = Column(UUID(as_uuid=True), ForeignKey('users.id'))
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    updated_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class Notification(Base):
    ...
    related_entity_type = Column(sa.Enum(RelatedEntityType))
...
class WorkerMedia(Base):
    ...
    media_type = Column(sa.Enum(MediaType), nullable=False)
...
class UserDashboardAccess(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    dashboard_type = Column(sa.Enum(DashboardType), nullable=False)
...
class NotificationPreference(Base):
    ...
    channel = Column(sa.Enum(NotificationChannel), nullable=False, default=NotificationChannel.IN_APP)
    created_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    updated_at = Column(sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
...
class FinancialEntry(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    factory_id = Column(UUID(as_uuid=True), ForeignKey('factories.id'), nullable=False)
    entry_type = Column(sa.Enum(ExpenseType), nullable=False)
    category = Column(sa.Enum(ExpenseCategory), nullable=False)
...
class InventoryReconciliation(Base):
    ...
    status = Column(sa.Enum(ReconciliationStatus), nullable=False, default=ReconciliationStatus.IN_PROGRESS)
...
class QmBlock(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    factory_id = Column(UUID(as_uuid=True), ForeignKey('factories.id'), nullable=False)
    block_type = Column(sa.Enum(QmBlockType), nullable=False)
    position_id = Column(UUID(as_uuid=True), ForeignKey('order_positions.id'))
    batch_id = Column(UUID(as_uuid=True), ForeignKey('batches.id'))
...
class SecurityAuditLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    action = Column(sa.Enum(AuditActionType), nullable=False)
...
class IpAllowlist(Base):
    ...
    scope = Column(sa.Enum(IpScope), nullable=False)
